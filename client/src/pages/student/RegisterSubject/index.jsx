...
const RegisterSubject = () => {
  ...
  useEffect(() => {
    ...
    const success =
      ...
      paymentTransaction.TransactionStatus == 0;
    if (success) {
      var feeDetails = selectedValues.map((item) => ({
        paymentTransactionId: paymentTransaction.Id,
        subjectId: item,
      }));
      studentApi.registerMultiSubject(feeDetails).then(() => {
      ...
      <Col span={12} style={{ textAlign: "center" }}>
        {!paymentTransaction && (
          <Space direction="vertical" style={{ width: "100%" }}>
            ...
            />
            <RegisterSubjectDetail
              selectedSubject={subjects.filter((s) =>
                selectedValues.includes(s.id)
              )}
            />
            <RegisterSubjectTotal
        ...
        {paymentTransaction && <Result {...getPaymentResultProps()} />}
      </Col>
      <Col span={6} />
    </Row>
  );
