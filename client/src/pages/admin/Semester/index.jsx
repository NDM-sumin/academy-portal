...
import StartNewSemesterButton from "./StartNewSemesterButton";
...
const Semester = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => `${record.startDay}/${record.startMonth}`,
    },
    {
      title: "Ngày kết thúc học kì",
      render: (_, record) => `${record.endDay}/${record.endMonth}`,
    },
  ];
      ...
      const mappedData = {
        ...
        items: response.items.map((item) => {
          return {
            ...
            duration: [
              dayjs(
                `${dayjs().year()}/${String(item.startMonth).padStart(
                  2,
                  "0"
                )}/${String(item.startDay).padStart(2, "0")}`,
                "YYYY-MM-DD"
              ),
              dayjs(
                `${dayjs().year()}/${String(item.endMonth).padStart(
                  2,
                  "0"
  ...
  const additionButtons = [
    <StartNewSemesterButton key={'dfsdfsdfs'}/>
  ...
  const formItems = [
    ...
    <Form.Item key={4} label="Học kì trước đó" name="prevSemesterId">
      <Select
        allowClear
        options={semesters
          .filter((item) => {
            const notHaveNextSmt = !item.nextSemester;
    ...
    <Form.Item
      ...
      rules={[
        {
          required: true,
          message: "Vui lòng chọn khoảng thời gian của học kì",
        },
      ]}
    >
